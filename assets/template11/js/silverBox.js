...
function silverBoxButtonComponent(buttonName, uniqClass, defaultText) {
	...
	if (buttonName.borderColor)
		button.style.borderColor = buttonName.borderColor;
	...
	// if closeOnClick in config is false the code will be executed
		...
		// loading animation
		if (
			buttonName.loadingAnimation !== false &&
			!buttonName.loadingAnimation
		) {
...
function silverBoxInputComponent({
	...
	if (!placeHolderFontSize) {
		if (fontSize)
			inputEl.style.setProperty(
				"--silverBox-placeHolder-fontSize",
				fontSize
			);
	} else {
		if (placeHolderFontSize)
	...
	if (className)
		className
			.split(" ")
			.forEach((className) => inputEl.classList.add(className));
	if (id) inputEl.id = id;
 ...
 */
function silverBoxCreateCustomIcon(
	customIcon,
	isCentred,
	className,
	id,
...
function silverBoxHeaderComponent({
	...
	if (
		(titleConfig?.customIcon && titleConfig?.alertIcon) ||
		(titleConfig?.customIcon && titleConfig?.customSvgIcon) ||
		titleConfig?.customIcon
	) {
		// stores returned customIcon element into a variable
		let customIcon = silverBoxIconsComponent({
		...
		if (titleConfig?.customIconId)
			customIcon.children[0].parentElement.id = titleConfig?.customIconId;
		...
		if (titleConfig?.customIconClassName)
			titleConfig?.customIconClassName.split(" ").forEach((className) => {
				customIcon.children[0].parentElement.classList.add(className);
			});
		...
		let alertIcon = silverBoxIconsComponent({
	...
	// customSvgIcon
		...
		let customSvgIcon = silverBoxIconsComponent({
			customSvgIcon: titleConfig?.customSvgIcon,
		});

		// if titleCustomIcon id exists, the img element of the customIcon Wrapper will receive given Id
		if (titleConfig?.customSvgIconId)
			customSvgIcon.children[0].parentElement.id =
				titleConfig?.customSvgIconId;

		// if titleCustomIcon class exists, the img element of the customIcon Wrapper will receive given class
		if (titleConfig?.customSvgIconClassName)
			titleConfig?.customSvgIconClassName
				.split(" ")
				.forEach((className) => {
					customSvgIcon.children[0].parentElement.classList.add(
						className
					);
				});
...
function silverBoxRemoveLoadings(animationIndex) {
	...
	let silverBoxButtonWrapper = document.querySelectorAll(
	...
	if (animationIndex === "last")
		animationIndex = silverBoxButtonWrapper.length;
	...
	// removes the nth modal's button's loading
		...
		silverBoxButtonWrapper[Number(animationIndex) - 1].childNodes.forEach(
			(button) => {
				button.classList.remove("silverBox-loading-button");
			}
		);
...
function silverBox(config) {
	...
	if (Object.keys(config).length !== 0) {
		...
		if (headerComponentConfig.length !== 0)
			elementsArray.push(headerComponentConfig);
		...
		for (const button of buttonsConfig) {
			if (
				button.type in config &&
				config[button.type].showButton !== false
			) {
		...
		let silverBoxWrapper = document.querySelectorAll(
			".silverBox-container"
		);
		...
		if ("silverBoxClassName" in config)
			config.silverBoxClassName
				.split(" ")
				.forEach((className) =>
					silverBoxWrapper.classList.add(className)
				);
	}
}
