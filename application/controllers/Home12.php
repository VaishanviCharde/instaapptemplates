...
class Home12 extends CI_Controller {

	public function __construct()
	{
	  	parent::__construct();
	  	$this->load->model('Home_model12');
	}
	...
	public function checklogin() {
        ...
        $apiResponse = $this->Home_model12->CallAPI($method, $url, $header);
		...
		if(isset($apiDecodedResponse->status) && $apiDecodedResponse->status == 200) {
			...
			/* Check Cart Available */
				...
				// );
				// $apiResponse2 = $this->Home_model12->CallAPI($method2, $url2, $header2);
				...
				// 	$details3 = $this->Home_model12->CallAPI($method3, $url3, $header3,json_encode($cart_data));
				...
				// 	'user_id: '.$customerId
				// );
				// $cart_list_details = $this->Home_model12->CallAPI($method1, $url1, $header1);
	...
	public function setAppConfigureData($appCommData) {
		...
		$temp1 = array(
			...
			'HEAD' => array(
				'C_TITLE' => $appName,
				'TEMP_LOGO' => $appLogo,
				'TEMP_TITLE' => ucfirst($appName).' - '.ucfirst($tempIndType),
				'TEMP_MDES' => ucfirst($appName).' - '.ucfirst($tempIndType),
			...
			'BANNER' => array(
				array(
					...
					'alt' => 'Banner Img 1',
					'sub-title' => '',
					'head' => 'We Serve Delicious <br>  Food for Customer',
				...
				array(
					...
					'alt' => 'Banner Img 2',
					'sub-title' => '',
					'head' => 'Good Food for <br>  Better Health',
					'desc' => 'Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore.',
					'isImgLeft' => 'true',
					'attr' => "target='_blank'",
					'link' => 'javascript:void(0)',
			...
			'ABOUT' => array(
				...
				'sub-title' => 'About Us',
				'head' => 'Know More About <br class="d-none d-md-block">  Our Restaurant',
				'desc' => $appDesc,
				'points' => array(
				...
				'onr_name' => 'Jerry Henson',
				'onr_designation' => '/ Restaurant Director',
				'authorSignImg' => 'author-sign.png',
			),
			'CALLSEC' => array(
				'SUP-MOB' => $appPhone, 
				'img' => array(
					array(
						'img' => '2.png',
						'cls' => 'ltn__call-to-4-img-1',
						'alt' => '',
					),
					array(
						'img' => '11.png',
						'cls' => 'ltn__call-to-4-img-2',
						'alt' => '',
					),
				),
			),
	...
	/** Page Start */
		...
		/** Sign Up / Sign In / Guest Login Action */
			public function signUpAction() {
				...
				$user_details = $this->Home_model12->CallAPI($method, $url, $header,json_encode($user_data));
				...
				if(isset($apiDecodedResponse->status) && $apiDecodedResponse->status == 200 && $apiDecodedResponse->status != NULL && $apiDecodedResponse->status != ""){
					...
					$cust_details = $this->Home_model12->CallAPI($method, $url, $header,json_encode($cust_data));
			...
			public function signInAction() {
				...
				$login_details = $this->Home_model12->CallAPI($method, $url, $header,json_encode($data));
				...
				if(isset($apiDecodedResponse->status) && $apiDecodedResponse->status == 200 && $apiDecodedResponse->status != NULL && $apiDecodedResponse->status != "") {
					...
					$cart_list_details = $this->Home_model12->CallAPI($method, $url, $header);
			...
			public function OTPVerifyAction() {
				...
				$details = $this->Home_model12->CallAPI($method, $url, $header,json_encode($data));
		...
		/** Product Details */
			public function product($catId = null)
			{
				...
				$apiResponse1 = $this->Home_model12->CallAPI($method1, $url1, $header1);
				...
				$cart_list_details = $this->Home_model12->CallAPI($method2, $url2, $header2);
			...
			public function getProductsByCategory() {
				...
				$apiResponse = $this->Home_model12->CallAPI($method1, $url1, $header1);
				$apiDecodedResponse = json_decode($apiResponse);
				$productList = array();
				if(isset($apiDecodedResponse->status) && $apiDecodedResponse->status == 200) {
			...
			public function addToCart() {
				...
				$apiResponse = $this->Home_model12->CallAPI($method, $url, $header,json_encode($cart_item_arr));
